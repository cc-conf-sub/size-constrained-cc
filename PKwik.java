...
public class PKwik {
    ...
    private static Integer[] leastNeighbors (ArrayList<ArrayList<Integer>> prob_matrix, int max_size, ArrayList<Integer> cur_cluster) {
        // return array of {node, num_neighbors}
        // TODO: make more efficient using hashmaps and heaps

        int minNeighbors = cur_cluster.size();
        Integer min_node = -1;
        for (int i = 0; i < cur_cluster.size(); i++) {
            int cur_neighbors = 0;
            int cur_node = cur_cluster.get(i);
            for (int j = 0; j < cur_cluster.size(); j++) {
                int other_node = cur_cluster.get(j);
                if (cur_node != other_node) {
                    if (prob_matrix.get(cur_node).contains(other_node))
                        cur_neighbors += 1; // IT'S A MATCH!                    
                }
            }
            if (cur_neighbors <= minNeighbors) {
                minNeighbors = cur_neighbors;
                min_node = cur_node;
            }

        }

        Integer[] result = {min_node, minNeighbors}; 
        return result;
    ...
    // LARGE NETWORK: READ AS ADJACENCY LIST
    public static ArrayList<ArrayList<Integer>> pivotAndAdjust(ArrayList<ArrayList<Integer>> prob_matrix, int max_size) {
        /*
            prob_matrix is now a dictionary of lists 
            index of graph is a node, list contains all neighbors of node
        */
        ...
        while (!available_nodes.isEmpty()) {
            ...
            // STEP 1:  FORM PIVOT CLUSTER JUST LIKE NORMAL
            for (int neighbor : prob_matrix.get(cur_node)) {
                if (available_nodes.contains(neighbor)) {
                    cur_cluster.add(neighbor);
                }
            ...
            // STEP 2:  IF PIVOT CLUSTER TOO LARGE, VOTE NODES OFF THE ISLAND
            if(cur_cluster.size() > max_size) {

                int min_neighbors = 0;
                while (cur_cluster.size() > max_size || min_neighbors < (cur_cluster.size() + 1) / 2) {

                    Integer[] result = leastNeighbors(prob_matrix, max_size, cur_cluster);
                    cur_cluster.remove(result[0]);
                    min_neighbors = result[1];
